...
class AccountManagementModel:
    ...
    def get_all_accounts(self):
        ...
        print(f"DEBUG: Total accounts in data manager: {len(accounts)}")
        for acc in accounts:
            print(f"  - {acc['name']}: closed={acc.get('closed', False)}")
        return accounts
    
    def get_active_accounts(self):
        """Get only active (non-closed) accounts"""
        active = [acc for acc in self.get_all_accounts() if not acc.get('closed', False)]
        print(f"DEBUG: Active accounts: {len(active)}")
        return active
    
    def get_closed_accounts(self):
        """Get only closed accounts"""
        closed = [acc for acc in self.get_all_accounts() if acc.get('closed', False)]
        print(f"DEBUG: Closed accounts: {len(closed)}")
        return closed
    ...
    def add_account(self, name, account_type, starting_balance=0.0, interest_rate=None, minimum_payment=None, payment_due_day=None):
        """
        Add a new account
    ...
    def update_account(self, old_name, new_name, account_type, starting_balance=0.0, interest_rate=None, minimum_payment=None, payment_due_day=None):
        """
